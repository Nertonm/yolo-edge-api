...
Dedicado à captura de dataset: serve o feed cru da câmera (sem bounding
boxes nem OSD) para acompanhar ao vivo no navegador enquanto o
capture_frames.py salva os frames em disco.

Expõe dois endpoints:
/stream    -> MJPEG contínuo, para visualização no navegador
/snapshot  -> um único frame JPEG por requisição, sem buffer acumulado
              (usado pelo capture_frames.py --manual)
...
def main():
    ...
    print(f"[INFO] Snapshot (usado pelo --manual): "
