...
def _read_next_frame(proc, leftover: bytes):
    """Le o proximo frame JPEG completo do stdout do rpicam-vid."""
    buf = leftover
    while True:
...
def main():
    ...
    while frame_count < args.frames:
        ...
        frame_count   += 1
        frames_captured += 1
        # V1 processa os frames em ordem; nao há fila de descarte.
    ...
    print(f"  Frames descartados: {frames_dropped} "
          f"({100*frames_dropped/max(frames_captured,1):.0f}%)")
    print("=" * 58)
    print()
    print("DIAGNÓSTICO:")
