...
class CameraCapture:
    ...
    def _capture_loop(self):
        ...
        Guarda apenas o JPEG bruto do frame mais recente (bytes).
        A decodificação só acontece em read(), no frame que será
        realmente processado pelo consumer. Assim a thread de captura
        não decodifica frames que seriam descartados e não rouba CPU
        da inferência.
        """
        raw = b""
        while self._running.is_set():
            ...
            if not chunk:
                ...
                self._capture_error = (
                    ...
                    else f"rpicam-vid falhou com exit_code={returncode}"
                )
                self._running.clear()
                break
            raw += chunk
            ...
            # Sobrescreve o frame anterior: guarda bytes, nao ndarray
            if self._buffer.full():
                try:
                    ...
                    pass
            self._buffer.put(jpg)

    def read(self, timeout: float = 1.0):
        """Retorna o frame mais recente (decodifica o JPEG) ou None se nenhum.

        Usa get_nowait + sleep curto: preserva o comportamento de pegar o
        frame mais recente do buffer sem re-decodificar os descartados.
        """
        deadline = time.monotonic() + timeout
        got = None
        while got is None:
            try:
                got = self._buffer.get_nowait()
            except queue.Empty:
                ...
                if time.monotonic() >= deadline:
                    return None
                time.sleep(0.001)
        frame = cv2.imdecode(np.frombuffer(got, dtype=np.uint8), cv2.IMREAD_COLOR)
        return frame
